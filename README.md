...
一个针对 OKX 交易所 SOL-USDT 永续合约的自动化交易机器人，支持**斥波那契网格策略**和**传统网格策略**、动态止盈、SQLite 数据库存储和 Telegram 通知。

## 功能特点

- **斥波那契网格策略**: 根据斥波那契数列自动计算目标持仓，动态买卖
- **传统网格策略**: 根据价格跌幅自动分批买入，固定张数
...
斥波那契策略根据当前价格自动计算目标持仓，在关键斥波那契点位触发买卖。

### 配置参数

| 参数 | 说明 | 默认值 |
|-----|------|-------|
| `FIBONACCI_ENABLED` | 是否启用斥波那契策略 | false |
...
1. **启动时**: 根据当前价格计算目标持仓，自动买入到目标数量
2. **价格上涨**: 突破斥波那契点位时卖出，减少持仓到目标数量
3. **价格下跌**: 跌破斥波那契点位时买入，增加持仓到目标数量

### 示例

```
启动，价格 $130 → 买入 20 张
涨到 $137.08 → 卖出 5 张（剩 15 张）
涨到 $145.84 → 卖出 6 张（剩 9 张）
回调到 $137.08 → 买入 6 张（回到 15 张）
继续跌到 $122.92 → 买入 9 张（24 张）
```
